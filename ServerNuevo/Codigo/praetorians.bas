...
Dim daño As Long
            ...
            If Npclist(npcind).Stats.MinMan > Hechizos(25).ManaRequerido Then
                If CInt(Rnd * (mankismo * mult + 1)) < 2 Then
                    ...
                    daño = RandomNumber(Hechizos(25).MinHP, Hechizos(25).MaxHP)
                    ...
                    Npclist(BestTarget).Stats.MinHP = Npclist(BestTarget).Stats.MinHP - daño
                    'Muere
                    Call SendData(SendTarget.ToNPCArea, BestTarget, PrepareMessageCreateHIT(Npclist(BestTarget).Char.CharIndex, daño, vbRed))
            ...
            ElseIf Npclist(npcind).Stats.MinMan > Hechizos(23).ManaRequerido Then
                If CInt(Rnd * (mankismo * mult + 1)) < 2 Then
                    ...
                    daño = RandomNumber(Hechizos(23).MinHP, Hechizos(23).MaxHP)
...
Dim daño As Long
                daño = RandomNumber(Hechizos(15).MinHP, Hechizos(15).MaxHP)
...
Dim daño As Integer
    ...
    If Hechizos(Spell).SubeHP = 1 Then
        ...
        Call WriteConsoleMsg(UserIndex, Npclist(NpcIndex).name & " repuesto " & daño & " puntos de vida.", FontTypeNames.FONTTYPE_FIGHT)

    ElseIf Hechizos(Spell).SubeHP = 2 Then

        daño = RandomNumber(Hechizos(Spell).MinHP * 2, Hechizos(Spell).MaxHP * 2)
        Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessagePlayWave(Hechizos(Spell).WAV, UserList(UserIndex).Pos.x, UserList(UserIndex).Pos.y))
        Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageCreateFX(UserList(UserIndex).Char.CharIndex, Hechizos(Spell).FXgrh, Hechizos(Spell).loops))
...
Dim Mapa As Integer
    ...
    'If (NPCx = x And NPCy = y) Then Exit Sub    ''ya llegué!!':( --> replaced by:
    If (NPCx = x And NPCy = y) Then
        Exit Sub    ''ya llegué!!
    End If
