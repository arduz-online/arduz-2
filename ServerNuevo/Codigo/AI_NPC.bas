...
'                            GreedyWalkTo NpcIndex, .Pos.map, UserList(UI).Pos.X, UserList(UI).Pos.Y
'                            Exit Sub
'                        End If
'
'                    End If
...
On Error GoTo ErrorHandler
    With Npclist(NpcIndex)
        If .Muerto = 1 Then
            .Contadores.TimeDead = .Contadores.TimeDead - 1
            If .Contadores.TimeDead = 0 Then
        ...
        If .MaestroUser = 0 And .Bot.BotType = 0 Then
        '[/MODIFICADO] Sistema de Bots MaTeO
