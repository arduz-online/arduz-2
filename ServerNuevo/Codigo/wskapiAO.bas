...
'        Dim lena As Long
'        Dim p As Long
'        Dim ACT_PA_LONG As Integer
'        Dim ba() As Byte
'        Do
'            CopyMemory ACT_PA_LONG, datos(0), 2
'
'            t = CRCPAXOR(datos(p + 2), ACT_PA_LONG + 1, .Redundance, 1)
'
'            ReDim ba(ACT_PA_LONG)
'            CopyMemory ba(0), datos(p + 2), ACT_PA_LONG + 1
'
'            Call .incomingData.WriteBlock(ba) '"PAQUETE(" & p + 2 & ";" & p + ACT_PA_LONG + 2 & "  of " & leng & ")[" & ACT_PA_LONG & "] =
'
'            Debug.Print "{" & .incomingData.get_hex_barray & "}"
'            p = p + ACT_PA_LONG + 2
'            lena = (lena - ACT_PA_LONG) - 2
'
'        Loop While lena > 2
        'print_barray datos
