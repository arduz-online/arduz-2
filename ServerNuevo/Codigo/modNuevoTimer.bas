...
If TActual - UserList(UserIndex).Counters.TimerLanzarSpell >= bClases(UserList(UserIndex).clase).intervalos(3) Then
    If Actualizar Then
...
If TActual - UserList(UserIndex).Counters.TimerPuedeAtacar >= bClases(UserList(UserIndex).clase).intervalos(5) Then
    If Actualizar Then
...
If TActual - UserList(UserIndex).Counters.TimerUsar >= Abs(bClases(UserList(UserIndex).clase).intervalos(0) - 100) Then
    If Actualizar Then UserList(UserIndex).Counters.TimerUsar = TActual
    IntervaloPermiteUsar = True
Else
    IntervaloPermiteUsar = False
    If (TActual - UserList(UserIndex).Counters.TimerUsar) < 20 Then Call SendData(SendTarget.ToGM, 0, PrepareMessageConsoleMsg("Servidor> " & UserList(UserIndex).name & " USO CON INTERVALO " & (TActual - UserList(UserIndex).Counters.TimerUsar), FontTypeNames.FONTTYPE_SERVER))
End If
...
If TActual - UserList(UserIndex).Counters.TimerPuedeUsarArco >= bClases(UserList(UserIndex).clase).intervalos(4) Then
