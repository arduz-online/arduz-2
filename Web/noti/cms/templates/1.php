...
function ver_lista(){
	...
	while($entrada = mysql_fetch_array($r)){
		...
		$categorias[$entrada['cat_nom']]['texto'] .=  '	- <a href="manual_'.urls_amigables($entrada['cat_nom'].'-'.$entrada['titulo']).'_'.$entrada['ID'].'.php">'.$entrada['titulo'].'</a><br/>';
	}
