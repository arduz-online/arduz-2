...
class Personajes{
	...
	function detect_cheat($puntos,$frags,$muertes){
		...
		if( $pcid!=0 ){
			...
			if( $puntos>44725 ){
				$database->uquery('UPDATE `sessions` SET `BAN`=`BAN`+5,`numcheats`=`numcheats`+1 WHERE `PCID`=\''.$Personajes->udata['PCID'].'\' LIMIT 1;');
			}
...
//$database->query("INSERT INTO `noicoder_sake`.`ban_log` (`ID` ,`uid` ,`gm` ,`tiempo` ,`razon`,`time`) VALUES (NULL , '$infoPJ[UID]', '$session->uid', '$tiempo', '<b>$session->username</b>: $_POST[razon]','".time()."');");
	function user_upd(){
		...
		$query		= $database->query("SELECT keysec FROM `servers` WHERE `ID` = '$server_id_str' LIMIT 1");
			if( mysql_num_rows($query)>0 ){
				...
				if( $serverdata[0]===$res['keysec'] ){
					...
					echo "'".$serverdata[0].DELIMITER.$res['keysec'].DELIMITER.$temp2;
				}
			} else {
				echo ')';
			}
