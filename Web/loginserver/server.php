 ...
 */
	...
	if( !empty($_POST['datos']) && $_REQUEST['a']==='crear' )
	{
		...
		if( mysql_num_rows($query)===0 ){
			...
			$query = $database->query("INSERT INTO `servers` SET `keysec`='$codigo',`ultima`='".$timenow."',`inicio`='".time()."', `IP` = '$IP',`Nombre`='$serverdata[0]',`Mapa`='$serverdata[1]',`Players`='$serverdata[2]',`PORT`='$serverdata[3]',`hamachi`='$serverdata[4]', `HOST`='$hostname', `maxusers`='$serverdata[6]', `passwd`='$serverdata[7]', `pcid`='$pc_acc_id'");
