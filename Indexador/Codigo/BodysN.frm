VERSION 5.00
Begin VB.Form CuerpoN 
   BackColor       =   &H00000000&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "[CUERPOS NATURALES]"
   ClientHeight    =   5730
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   5895
   Icon            =   "BodysN.frx":0000
   LinkTopic       =   "Form2"
   MaxButton       =   0   'False
   ScaleHeight     =   5730
   ScaleWidth      =   5895
...
Frames = Frames + 1
If Frames > 4 Then Frames = 1
For i = 1 To 4
    Gr = BodyData(Listado.Text).Walk(i).GrhIndex
    Call CargarImagen(i - 1, Gr, Frames)
    Paso(i) = Paso(i) + 1
...
Selecc = 0
Me.Icon = Form1.Icon
Me.Caption = Form1.Caption & " [CUERPOS NATURALES]"
Listado.Clear
...
Next
GRHt.Text = Left(GRHt.Text, Len(GRHt.Text) - 1)
End Sub
...
Sub CargarImagen(ByVal Index As Integer, ByVal GrhIndex As Integer, ByVal Frame As Integer)
    ...
    ImagenX = App.Path & "\GRAFICOS\" & GrhData(GrhIndex).FileNum & ".BMP"
