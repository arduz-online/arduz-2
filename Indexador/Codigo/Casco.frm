...
If Selecc <= 0 Or Selecc > NumCascos Then Exit Sub
Paso = Paso + 1
Ix.Visible = False
...
Ix.Left = 0
Ix.Top = 0
Gr = CascoAnimData(Selecc).Head(Paso).GrhIndex
...
Selecc = 0
Me.Icon = Form1.Icon
Me.Caption = Form1.Caption & " [CASCOS]"
Listado.Clear
...
Dim i As Integer, Gr As Integer
Dim ImagenX As String
GRHt.Text = "[CASCO" & Listado.Text & "]" & vbCrLf & vbCrLf
Img.Visible = False
Selecc = Listado.Text
Paso = 0
For i = 0 To 3
    ...
    HDX(i).Left = 0
    HDX(i).Top = 0
    Gr = CascoAnimData(Listado.Text).Head(i + 1).GrhIndex
    GRHt.Text = GRHt.Text & "Casco" & (i + 1) & "=" & Gr & vbCrLf
    HX(i).Caption = Gr
    ImagenX = DirClien & "\GRAFICOS\" & GrhData(Gr).FileNum & ".BMP"
    If LenB(Dir(ImagenX, vbArchive)) = 0 Then Exit Sub
