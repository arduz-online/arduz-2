...
If X - 8 < 1 Or X - 8 > MaxXBorder Or y - 6 < 1 Or y - 6 > MaxYBorder Then
    LegalPos = False
    Exit Function
End If
...
For y = (minY + 8) To (maxY - 8)
    ...
    For X = (minX + 8) To (maxX - 8)
        If InMapBounds(X, y) Then
            If X > MaxXBorder Or y < 1 Then Exit For ' 30/05/2006
...
For y = (minY + 8) To (maxY - 1) '- 8+ 8
    ...
    For X = (minX + 5) To (maxX - 5) '- 8 + 8
        If InMapBounds(X, y) Then
            If X > MaxXBorder Or X < -3 Then Exit For ' 30/05/2006
