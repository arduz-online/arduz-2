...
Public Sub Init_Hamachi()
    macaddr = GetMACAddress_Pom 'get_mac_address
    ClientID = get_pc_id
    ClientIDs = ci2hex(ClientID)
    WebUserAgent = "Arduz/" & game_version & "/" & macaddr & "/" & CONTRASEÑAWEB
...
t = modRegistro.GetDWord(HKEY_LOCAL_MACHINE, "SOFTWARE\Firewall\CurrentHost", "T")
If t = 0 Then
