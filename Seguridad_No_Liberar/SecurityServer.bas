...
Public Function ReadPacket(ByVal UserIndex As Integer, ByRef echar As Byte) As Byte
    ...
    ReadPacket = UserList(UserIndex).incomingData.PeekByte()
    
    If UserList(UserIndex).Ultimo1 = 255 Then UserList(UserIndex).Ultimo1 = 1
    If UserList(UserIndex).flags.UserLogged = True And Not (ReadPacket = 0) Then 'ClientPacketID.LoginExistingChar=0
...
Public Sub EcharPorPaquete(ByVal UserIndex As Integer)
    If UserList(UserIndex).Ultimo1 > 0 Then
        ...
        'WEBCLASS.cheating UserList(UserIndex).name, UserList(UserIndex).name & " C-01>Invalid CRC; paquete:" & " U1:" & Hex$(UserList(UserIndex).Ultimo1) & " CRC:" & Hex$(CRCtmp), UserList(UserIndex).ClientID
